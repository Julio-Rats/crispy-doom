...
void WI_drawLF(void)
{
    ...
    {
        // draw <LevelName>
        V_DrawPatch((ORIGWIDTH - SHORT(lnames[wbs->last]->width)) / 2, y,
                    lnames[wbs->last]);
...
void WI_initShowNextLoc(void)
{
    ...
    if ((gamemode != commercial && gamemap == 8) ||
        (gameversion == exe_chex && gamemap == 5))
    {
        G_WorldDone();
        return;
    }
...
void WI_updateDeathmatchStats(void)
{
    ...
    if (acceleratestage && dm_state != 4)
    {
        ...
        S_StartSoundOptional(0, sfx_inttot,
                             sfx_barexp); // [NS] Optional inter sounds.
        dm_state = 4;
    }
    ...
    {
        if (!(bcnt & 3))
            S_StartSoundOptional(0, sfx_inttic,
                                 sfx_pistol); // [NS] Optional inter sounds.
    ...
    else if (dm_state == 4)
    {
        if (acceleratestage)
        {
            S_StartSoundOptional(0, sfx_intdms,
                                 sfx_slop); // [NS] Optional inter sounds.
...
void WI_updateNetgameStats(void)
{
    ...
    else if (ng_state == 8)
    {
        ...
        if (!stillticking)
        {
            S_StartSoundOptional(0, sfx_intnet,
                                 sfx_pldeth); // [NS] Optional inter sounds.
            ng_state++;
        }
    }
    else if (ng_state == 10)
    {
        if (acceleratestage)
        {
            S_StartSoundOptional(0, sfx_intnex,
                                 sfx_sgcock); // [NS] Optional inter sounds.
            if (gamemode == commercial)
                WI_initNoState();
            else
                WI_initShowNextLoc();
...
void WI_drawStats(void)
{
    ...
    if (sp_state > 8)
    {
        ...
        WI_drawTime((wide ? ORIGWIDTH : ORIGWIDTH / 2) - SP_TIMEX,
...
void WI_Ticker(void)
{
    ...
    if (bcnt == 1)
    {
        ...
        // [crispy] Sigil
        else if (crispy->haved1e5 && wbs->epsd == 4 &&
                 W_CheckNumForName(DEH_String("D_SIGINT")) != -1)
            S_ChangeMusic(mus_sigint, true);
        // [crispy] Sigil II
        else if (crispy->haved1e6 && wbs->epsd == 5 &&
                 W_CheckNumForName(DEH_String("D_SG2INT")) != -1)
...
static void WI_loadUnloadData(load_callback_t callback)
{
    ...
    if (gamemode == commercial)
    {
        for (i = 0; i < NUMCMAPS; i++)
        {
            ...
            if (crispy->havenerve && wbs->epsd == 1 &&
                i < 9) // [crispy] gamemission == pack_nerve
            {
                name[0] = 'N';
            }
            if (crispy->havemaster && crispy->havemaster != (char *) -1 
               && wbs->epsd == 2 && i < 21) // [crispy] gamemission == pack_master
            {
                name[0] = 'M';
            }
    ...
    {
        if (crispy->havenerve && wbs->epsd == 1 &&
            W_CheckNumForName(DEH_String("NERVEINT")) !=
                -1) // [crispy] gamemission == pack_nerve
        {
            M_StringCopy(name, DEH_String("NERVEINT"), sizeof(name));
        }
        else if (crispy->havemaster && wbs->epsd == 2 &&
                 W_CheckNumForName(DEH_String("MASTRINT")) !=
                     -1) // [crispy] gamemission == pack_master
        {
    ...
    else if (gameversion >= exe_ultimate && wbs->epsd == 3)
    {
        M_StringCopy(name, DEH_String("INTERPIC"), sizeof(name));
             ...
             W_CheckNumForName(DEH_String("SIGILINT")) != -1) // [crispy] Sigil
    {
        M_StringCopy(name, DEH_String("SIGILINT"), sizeof(name));
    }
    else if (crispy->haved1e6 && wbs->epsd == 5 &&
             W_CheckNumForName(DEH_String("SIGILIN2")) != -1) // [crispy] Sigil
    {
